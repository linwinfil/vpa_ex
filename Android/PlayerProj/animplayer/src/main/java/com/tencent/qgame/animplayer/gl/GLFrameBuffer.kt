...
class GLFrameBuffer constructor(val bufferWidth: Int, val bufferHeight: Int, val bufferSize: Int = 1) {
    ...
    private val colorTextureArr: IntArray = IntArray(bufferSize)

    init {
        //创建frame buffer
        GLES30.glGenFramebuffers(bufferSize, bufferArr, 0)
        //创建挂载颜色的纹理
        GLES30.glGenTextures(bufferSize, colorTextureArr, 0)
    ...
    internal fun bindNext() {
        bindNext(GLES30.GL_NONE)
    }

    internal fun bindNext(textureId: Int) {
    ...
    fun onDestroy() {
        ...
        GLES30.glDeleteTextures(bufferSize, colorTextureArr, 0)
    }
    ...
    fun getCurrentTextureId():Int = colorTextureArr[currentIndex]
