 ...
 */
public class GPUImageRendererImpl implements GLSurfaceView.Renderer, GLTextureView.Renderer, Camera.PreviewCallback {
    ...
    public GPUImageRendererImpl(final GPUImageFilter filter) {
        this.filter = filter;
        ...
        runOnDraw(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    camera.setPreviewCallback(GPUImageRendererImpl.this);
                    camera.startPreview();
                } catch (IOException e) {
                    e.printStackTrace();
                }
    ...
    public void setFilter(final GPUImageFilter filter) {
        runOnDraw(new Runnable() {
            ...
            @Override
            public void run() {
                final GPUImageFilter oldFilter = GPUImageRendererImpl.this.filter;
                GPUImageRendererImpl.this.filter = filter;
                if (oldFilter != null) {
                    oldFilter.destroy();
                }
                GPUImageRendererImpl.this.filter.ifNeedInit();
                GLES20.glUseProgram(GPUImageRendererImpl.this.filter.getProgram());
                GPUImageRendererImpl.this.filter.onOutputSizeChanged(outputWidth, outputHeight);
            }
        });
    }
